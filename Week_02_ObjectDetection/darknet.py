...
class CNNBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # conv2d, batchnorm2d, leakyrelu를 차례로 적용하는 역할을 해야함
        x = self.conv(x)
        x = self.batchnorm(x)
        x = self.leakyrelu(x)
    ...
    def _create_conv_layers(self, architecture):
        ...
        for x in architecture:
            if type(x) == tuple:  
                ...
                #MaxpoolingLayer를 Maxpool2dLayer로 변환
                if x == "M":
                    layers.append(nn.MaxPool2d(kernel_size=2, stride=2))

            elif type(x) == list: 
                #list를 conv2d layer로 변환
                #첫번쨰 Convolution Layer
                layers.append(
                    CNNBlock(
                ...
                in_channels = conv2[1] #다음 레이어의 입력 채널로 설정
